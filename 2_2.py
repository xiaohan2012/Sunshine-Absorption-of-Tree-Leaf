...
class TreeGenerator(object):
    ...
    def gen_leaf(self,leaf_size_equal = False):
        _param = {}
    ...
    def gen_tree(self,leaf_size_equal = False):
        SunlitArea = namedtuple('SunlitArea','polygon light_strength')
        for i in xrange(leaf_cnt):
            leaf = self.gen_leaf(leaf_size_equal = leaf_size_equal)

            self.tree.addLeaf(leaf)

            for lf in self.tree.getAboveLeaves(leaf):
                for area in lf.getSunlitAreas():
                    if leaf.surface.intersects(area.polygon):#if the below leaf is not blocked by this area
                ...
                for above_area in leaf.getSunlitAreasAbove(area):
                    ...
                    if above_area.polygon.intersects(area.polygon):
                        ...
                        leaf.sunlit_areas.insert(index, SunlitArea(\
                                          polygon= above_area.polygon.difference(area.polygon).buffer(1),\
...
if __name__ == "__main__":
    ...
    tree = tg.gen_tree(20,leaf_size_equal = True)
    tree.show()
    #result = []
    #for ba_ratio in arange(0.01,0.4,0.01):
